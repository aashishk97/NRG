...
max_iter = 10             # maximum number of iterations (signals convergence failure)
...
def dmft_step(Delta_in):
  ...
  # Solve Parameters
  sp = { "T": T, "Lambda": 3.0, "Nz": 4, "Tmin": 1e-5, "keep": 10000, "keepenergy": 10.0 }
  ...
  for i, e_i in enumerate(disorder_values):
    sp["model_parameters"]["eps1"] = -mu + e_i
    ...
    for bl in Gself.indices:
        for w in Gself.mesh:
            Gself[bl][w] = np.linalg.inv( (w + mu)*identity(bl) - Delta_in[bl][w] - S.Sigma_w[bl][w] ) # !!!
    ...
    for bl in Gloc.indices:
        for w in Gloc.mesh:
            for i in range(block_size(bl)):
                for j in range(block_size(bl)): 
                    if i == j:
                        Gloc[bl][w][i,i] = ht(w + mu - S.Sigma_w[bl][w][i,i]) 
                    else:
                        assert abs(Sigma[bl][w][i,j])<1e-10, "This implementation only supports diagonal self-energy"
                        Gloc[bl][w][i,j] = 0.0
    Glocinv = Gloc.inverse()
    ...
    for bl in Delta.indices:
        for w in Delta.mesh:
            Delta[bl][w] = (w+mu)*identity(bl) - S.Sigma_w[bl][w] - Glocinv[bl][w] 
  ...
  save_rho(f"{itern}_rho.txt", Gloc)
  save_gf(f"{itern}_gloc.txt", Gloc)
  save_gf(f"{itern}_delta.txt", Delta)
  save_gf(f"{itern}_sigma.txt", S.Sigma_w)
